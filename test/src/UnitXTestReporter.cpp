...
UnitXTestReporter::UnitXTestReporter(string testsuiteName, ostream& ostream)
    : m_ostream(ostream), m_testSuiteName(testsuiteName)
{
}

void UnitXTestReporter::ReportSummary(int totalTestCount, int failedTestCount,
...
void UnitXTestReporter::AddXmlElement(ostream& os, char const* encoding)
{
    os << "<?xml version=\"1.0\"";
...
void UnitXTestReporter::BeginResults(std::ostream& os, int totalTestCount, int failedTestCount,
                                   int failureCount, float secondsElapsed)
{
   os << "<testsuite"
       << " name=\"" << m_testSuiteName << "\""
...
void UnitXTestReporter::EndResults(std::ostream& os)
{
    os << "</testsuite>";
}

void UnitXTestReporter::BeginTest(std::ostream& os, DeferredTestResult const& result)
{
    os << "<testcase"
...
void UnitXTestReporter::EndTest(std::ostream& os, DeferredTestResult const& result)
{
    if (result.failed)
        os << "</testcase>";
    else
...
void UnitXTestReporter::AddFailure(std::ostream& os, DeferredTestResult const& result)
{
